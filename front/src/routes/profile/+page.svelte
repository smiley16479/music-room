<svelte:head>
	...
	<meta name="description" content="Manage your profile and music preferences" />
</svelte:head>
...
<script lang="ts">
	...
	let error = '';
	let success = '';
	let activeTab: 'public' | 'friends' | 'private' | 'music' | 'accounts' = 'public';
	...
	const musicGenres = [
		...
		'R&B', 'Reggae', 'Blues', 'Folk', 'Indie', 'Metal', 'Funk', 'Soul'
	];
	...
	function populateFormData(userData: User) {
		...
		publicInfo = {
			displayName: userData.publicInfo?.displayName || userData.displayName || '',
			bio: userData.publicInfo?.bio || '',
			profilePicture: userData.publicInfo?.profilePicture || userData.profilePicture || ''
		};

		// Populate friends info
		friendsInfo = {
			realName: userData.friendsInfo?.realName || '',
			location: userData.friendsInfo?.location || '',
			favoriteGenres: userData.friendsInfo?.favoriteGenres || []
		};

		// Populate private info
		privateInfo = {
			phoneNumber: userData.privateInfo?.phoneNumber || '',
			birthDate: userData.privateInfo?.birthDate || ''
		};

		// Populate music preferences
		musicPreferences = {
			favoriteGenres: userData.musicPreferences?.favoriteGenres || [],
			favoriteArtists: userData.musicPreferences?.favoriteArtists || [],
			listeningHabits: userData.musicPreferences?.listeningHabits || ''
		};
	}
	...
	async function linkSocialAccount(provider: 'google' | 'facebook') {
		...
		try {
			...
			const token = prompt(`Enter your ${provider} authentication token:`);
			if (token) {
	...
	function addGenre(target: 'friends' | 'music') {
		...
		if (target === 'friends') {
			if (!friendsInfo.favoriteGenres.includes(newGenre)) {
				friendsInfo.favoriteGenres = [...friendsInfo.favoriteGenres, newGenre];
			}
		} else {
	...
	function removeGenre(genre: string, target: 'friends' | 'music') {
		if (target === 'friends') {
			friendsInfo.favoriteGenres = friendsInfo.favoriteGenres.filter(g => g !== genre);
		} else {
...
<div class="container mx-auto px-4 py-8">
	<div class="flex items-center justify-between mb-8">
		<div>
			<h1 class="font-family-main text-4xl font-bold text-gray-800 mb-2">Profile Settings</h1>
	...
	<div class="border-b border-gray-200 mb-8">
		<nav class="flex space-x-8">
			<button 
				class="py-2 px-1 border-b-2 font-medium text-sm {activeTab === 'public' ? 'border-secondary text-secondary' : 'border-transparent text-gray-500 hover:text-gray-700'}"
				on:click={() => activeTab = 'public'}
			>
				Public Information
			</button>
			<button 
				class="py-2 px-1 border-b-2 font-medium text-sm {activeTab === 'friends' ? 'border-secondary text-secondary' : 'border-transparent text-gray-500 hover:text-gray-700'}"
				on:click={() => activeTab = 'friends'}
			>
				Friends Information
			</button>
			<button 
				class="py-2 px-1 border-b-2 font-medium text-sm {activeTab === 'private' ? 'border-secondary text-secondary' : 'border-transparent text-gray-500 hover:text-gray-700'}"
				on:click={() => activeTab = 'private'}
			>
				Private Information
			</button>
			<button 
				class="py-2 px-1 border-b-2 font-medium text-sm {activeTab === 'music' ? 'border-secondary text-secondary' : 'border-transparent text-gray-500 hover:text-gray-700'}"
				on:click={() => activeTab = 'music'}
			>
				Music Preferences
			</button>
			<button 
				class="py-2 px-1 border-b-2 font-medium text-sm {activeTab === 'accounts' ? 'border-secondary text-secondary' : 'border-transparent text-gray-500 hover:text-gray-700'}"
	...
	<form on:submit|preventDefault={updateProfile}>
		...
		<div class="bg-white rounded-lg shadow-md p-6 mb-6">
			<h2 class="text-xl font-bold text-gray-800 mb-4">Public Information</h2>
			<p class="text-gray-600 mb-6">This information is visible to all users</p>
		...
		<div class="bg-white rounded-lg shadow-md p-6 mb-6">
			<h2 class="text-xl font-bold text-gray-800 mb-4">Friends Information</h2>
			<p class="text-gray-600 mb-6">This information is only visible to your friends</p>

			<div class="space-y-4">
				<div>
					<label for="real-name" class="block text-sm font-medium text-gray-700 mb-1">Real Name</label>
					<input 
						id="real-name"
						type="text" 
				...
				<div>
					<label for="friends-genres" class="block text-sm font-medium text-gray-700 mb-1">Favorite Genres</label>
					<div class="flex flex-wrap gap-2 mb-2">
						{#each friendsInfo.favoriteGenres as genre}
						<span class="inline-flex items-center px-3 py-1 rounded-full text-sm bg-secondary/20 text-secondary">
							{genre}
							<button 
								type="button"
								on:click={() => removeGenre(genre, 'friends')}
					...
					<div class="flex space-x-2">
						<input 
							...
							on:keydown={(e) => e.key === 'Enter' && (e.preventDefault(), addGenre('friends'))}
						/>
						<button 
							type="button"
							on:click={() => addGenre('friends')}
					...
					<div class="mt-2 flex flex-wrap gap-1">
						...
						<button 
							type="button"
							on:click={() => { newGenre = genre; addGenre('friends'); }}
		...
		<div class="bg-white rounded-lg shadow-md p-6 mb-6">
			<h2 class="text-xl font-bold text-gray-800 mb-4">Private Information</h2>
			<p class="text-gray-600 mb-6">This information is only visible to you</p>
		...
		{#if activeTab === 'music'}
		<div class="bg-white rounded-lg shadow-md p-6 mb-6">
			<h2 class="text-xl font-bold text-gray-800 mb-4">Music Preferences</h2>
			<p class="text-gray-600 mb-6">Help us recommend better music for you</p>

			<div class="space-y-6">
				<div>
					<label for="music-genres" class="block text-sm font-medium text-gray-700 mb-1">Favorite Genres</label>
					<div class="flex flex-wrap gap-2 mb-2">
						...
						<span class="inline-flex items-center px-3 py-1 rounded-full text-sm bg-purple-100 text-purple-700">
							...
							<button 
								type="button"
								on:click={() => removeGenre(genre, 'music')}
					...
					<div class="flex space-x-2">
						<input 
							...
							class="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary"
							placeholder="Add a genre"
							on:keydown={(e) => e.key === 'Enter' && (e.preventDefault(), addGenre('music'))}
						/>
						<button 
							type="button"
							on:click={() => addGenre('music')}
							class="px-4 py-2 bg-secondary text-white rounded-lg hover:bg-secondary/80"
						>
							Add
						</button>
					...
					<div class="mt-2 flex flex-wrap gap-1">
						...
						<button 
							type="button"
							on:click={() => { newGenre = genre; addGenre('music'); }}
							class="text-xs px-2 py-1 bg-gray-100 text-gray-600 rounded hover:bg-gray-200"
						>
							{genre}
						</button>
				...
				<div>
					<label for="favorite-artists" class="block text-sm font-medium text-gray-700 mb-1">Favorite Artists</label>
					<div class="flex flex-wrap gap-2 mb-2">
					...
					<div class="flex space-x-2">
						<input 
							...
							on:keydown={(e) => e.key === 'Enter' && (e.preventDefault(), addArtist())}
						/>
						<button 
							type="button"
							on:click={addArtist}
				...
				<div>
					<label for="listening-habits" class="block text-sm font-medium text-gray-700 mb-1">Listening Habits</label>
					<textarea 
						id="listening-habits"
		...
		<div class="bg-white rounded-lg shadow-md p-6 mb-6">
			...
			<p class="text-gray-600 mb-6">Link your social media accounts for easier access</p>
			...
			<div class="space-y-4">
				...
				<div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
					...
					<button 
						type="button"
						on:click={() => unlinkSocialAccount('google')}
					...
					<button 
						type="button"
						on:click={() => linkSocialAccount('google')}
				...
				<div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
					...
					<button 
						type="button"
						on:click={() => unlinkSocialAccount('facebook')}
						class="border border-red-500 text-red-500 px-4 py-2 rounded-lg hover:bg-red-50 transition-colors"
					>
						Disconnect
					</button>
					...
					<button 
						type="button"
						on:click={() => linkSocialAccount('facebook')}
